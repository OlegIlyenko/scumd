...
public abstract class BaseUserDaoAuthenticator {
    ...
    protected String checkUser(User user, String userName) {
        if (user == null) {
            return "Unknown user '" + GitUtil.renderUserName(userName) + "' tried to login.";
        }

        if (!user.isActive()) {
            return "Inactive user '" + GitUtil.renderUserName(userName) + "' tried to login.";
        }

        if (user.getExpirationDate() != null && user.getExpirationDate().before(new Date())) {
            return "Expired user '" + GitUtil.renderUserName(userName) + "' tried to login.";
        }

        if (user.getAuthPolicy() == null) {
            return "User '" + GitUtil.renderUserName(userName) + "' have no authentication policy defined!";
        }
    ...
    protected boolean processNoAuth(String userName, User user, InteractionContext ctx, SocketAddress address, ServerSession session) {
        ...
        log.debug("User '" + GitUtil.renderUserName(userName) + "' passed authentication because he has NoAuth policy!");
    ...
    protected void processAuthResult(String userName, User user, InteractionContext ctx, SocketAddress address, boolean success, String method) {
        if (log.isDebugEnabled()) {
            if (success) {
                log.debug("User '" + GitUtil.renderUserName(userName) + "' was successfully authenticated with " + method);
                ctx.getEventDispatcher().fireEvent(new AuthenticationSuccessEventImpl(user, ctx.getServer(), userName, address, method));
            } else {
                String authError = "User '" + GitUtil.renderUserName(userName) + "' failed authentication with " + method + "!";
                log.warn(authError);
